...
def detect_roast_ammo(profile: dict, repos: list, events: list) -> list[str]:
    ...
    # --- AI-assisted / LLM user detection ---
    ai_keywords = ["claude", "anthropic", "co-authored-by: claude", "gpt", "openai", "copilot", "chatgpt", "gemini", "cursor", "co-authored-by: github copilot", "generated by", "ai-generated", "llm"]
    ai_commits = []
    ai_repos = []
    for e in events:
        if e.get("type") == "PushEvent":
            for c in e.get("payload", {}).get("commits", []):
                ...
                if any(kw in msg for kw in ai_keywords):
                    ai_commits.append(c.get("message", "").split("\n")[0][:80])
    for r in repos:
        desc = (r.get("description") or "").lower()
        rname = (r.get("name") or "").lower()
        if any(kw in desc or kw in rname for kw in ai_keywords):
            ai_repos.append(r["name"])
    if ai_commits:
        samples = '", "'.join(ai_commits[:3])
        ammo.insert(0, f'AI-POWERED FRAUD: Found LLM/AI in commit messages ("{samples}"). This person doesn\'t write code, they PROMPT code. Their real skill is typing "please fix this" into ChatGPT. Claude is the real developer here, this person is just the project manager.')
    if ai_repos:
        names = ", ".join(ai_repos[:3])
        ammo.insert(0, f"AI REPOS DETECTED: Repos like {names} mention AI/LLM tools. Bro's entire GitHub is a receipt for their Claude/GPT subscription.")
